...
function checkout() {
  // Guardar flujo: si no hay datos del cliente, NO limpiar el carrito y salir
  try { customerData = JSON.parse(localStorage.getItem('customerData')) || null; } catch {}
  if (!customerData || !customerData.name || !customerData.lastname) {
    showToast('Por favor ingrese sus datos en la tienda antes de proceder al pago', 'warning');
    return; // Salimos ANTES de revisar stock o tocar el carrito
  }
const items = Cart.get();
  if (items.length === 0) {
